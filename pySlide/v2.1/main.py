...
class YouTubeTranscriptDownloader(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.output_format_dropdown.currentIndexChanged.connect(
            self.save_output_format)
        self.language_dropdown.currentIndexChanged.connect(self.save_language)
        self.file_policy_dropdown.currentIndexChanged.connect(
            self.save_file_policy)

        # Load Recent Downloads
        recent_downloads = load_recent_downloads()
        for item in recent_downloads:
            lang_tag = f"[{item.get('language', 'N/A').upper()}]"
            display_title = f"{item['title']} ({lang_tag}, {
                os.path.splitext(item['file_path'])[1][1:].upper()})"
    ...
    def populate_theme_dropdown(self):
        ...
        index = self.theme_dropdown.findData(current_theme)  # Find the index of the current theme
        if index >= 0:
    ...
    def apply_theme(self):
        ...
        themes_dir = resource_path("themes")
        if not os.path.exists(themes_dir):
            self.console_output.append("<span style='color: red;'>Themes folder missing. Applying fallback theme.</span>")
        ...
        if current_theme in themes:
            self.load_theme_from_file(current_theme, log_message=False)  
        else:
            self.console_output.append("<span style='color: red;'>Theme not found. Applying fallback theme.</span>")
            self.apply_fallback_theme()

    def load_theme_from_file(self, theme_name, log_message=False):
        ...
        if os.path.exists(theme_file):
            try:
                ...
                if log_message:
                    display_message(self.console_output, f"Applied theme: {theme_name}", "success")
            except Exception as e:
                display_message(self.console_output, f"Failed to load theme '{theme_name}': {e}", "error")
        else:
            self.apply_fallback_theme()
    ...
    def open_menu(self, position):
        ...
        action = menu.exec(self.recent_list.viewport().mapToGlobal(
            position))  # changed from exec_ to exec
                ...
                if file_path and os.path.exists(file_path):
                    try:
                        ...
                        display_message(self.console_output,
                                        f"Opened file: {file_path}", "info")
                    except Exception as e:
                        display_message(self.console_output,
                                        f"Failed to open file: {e}", "error")
                else:
                    display_message(self.console_output,
    ...
    def change_theme(self, index):
        ...
        save_settings(self.settings)
        display_message(self.console_output, f"Applied theme: {self.theme_dropdown.currentText()}", "success")
    ...
    def start_download(self):
        ...
        if not video_url:
            ...
            return
        display_message(self.console_output,
                        f"Starting download for: {video_url}")
        self.progress_bar.setValue(0)
    ...
    def cancel_download(self):
        ...
        self.cancel_button.setEnabled(False)
        display_message(self.console_output,
                        "Download cancelled by user.", "info")
